...
fun main() = runBlocking {
    // по дефолту будет выполняться с диспетчером родительской корутины
    launch { // context of the parent, main runBlocking coroutine
        println("main runBlocking      : I'm working in thread ${Thread.currentThread().name}") // thread main
    }.join()

    // Dispatchers.Unconfined - будет запущено в рамках того потока, в рамках которого оно было изначально запущено
    launch(Dispatchers.Unconfined) { // not confined to any thread -- will work with main thread now, and later on different
        println("Unconfined            : I'm working in thread ${Thread.currentThread().name}") // thread main
        delay(50L) // но после прерывания остальная часть корутины будет закончена в рамках любого другого свободного потока
        println("Unconfined            : I'm working in thread ${Thread.currentThread().name} after delay()") // thread kotlinx.coroutines.DefaultExecutor
    }.join()

    launch(Dispatchers.Default) { // will get dispatched to DefaultDispatcher -> shared background pool of threads
        println("Default               : I'm working in thread ${Thread.currentThread().name}") // thread DefaultDispatcher-worker-1
    }.join()

    launch(newSingleThreadContext("MyOwnThread")) { // will get its own new thread
        println("newSingleThreadContext: I'm working in thread ${Thread.currentThread().name}") // thread MyOwnThread
    }.join()
}
